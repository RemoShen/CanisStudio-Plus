...
export class SuggestPanel {
     ...
     * createSuggestPanel
     */
    public createSuggestPanel(allNextKf: string[][], height: number) {
        ...
        if (allNextKf.length <= 0) {
            ...
            this.suggestMenu.createSuggestMenu({ x: this.boxWidth, y: height / 2 + SuggestPanel.PADDING }, allNextKf, this);
        ...
        for (let i = 0; i < this.numShown; i++) {
            ...
            item.setAttributeNS(null, 'transform', `translate(0, ${i * (height + 2 * SuggestPanel.PADDING)})`);
            this.itemcontainer.appendChild(item);
        }

        const popupLayer: SVGElement = document.createElementNS('http://www.w3.org/2000/svg', 'g');
    ...
    public createSuggestItem(nextKf: string[]) {
        ...
        const img = itemImg.createItemimg();
        const bg: SVGRectElement = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
...
export class SuggestMenu {
    ...
    public createSuggestMenu(startCoord: ICoord, nextKf: string[][], SuggestPanel: SuggestPanel) {
    ...
    public arrowClickListener() {
        ...
        for (let i = this.pageIdx * 2; i < (this.pageIdx + 1) * 2; i++) {
            if (i < this.nextKf.length) {
                ...
                item.setAttributeNS(null, 'transform', `translate(0, ${index * (this.parentSuggestPanel.kfHeight + 2 * SuggestPanel.PADDING)})`);
...
export class Itemimg {
    ...
    public container: SVGElement;
    public createItemimg() {
        this.container = document.createElementNS('http://www.w3.org/2000/svg', 'g');
        const bg = this.drawItemBg(suggestPanel.kfHeight - 2 * SuggestPanel.PADDING);
        bg.setAttributeNS(null, 'transform', `translate(5, ${SuggestPanel.PADDING})`);
        //TODO: add image here
        this.container.appendChild(bg);
        //render svg to canvas on bg
